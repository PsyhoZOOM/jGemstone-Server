...
/**
 * The Mikrotik API connection. This is the class used to connect to a remote Mikrotik and send
 * commands to it.
 *
...
public abstract class ApiConnection implements AutoCloseable {
  ...
  /**
   * Create a new API connection to the give device on the supplied port using the supplied socket
   * factory to create the socket.
   *
   ...
   * @since 3.0
   */
  public static ApiConnection connect(SocketFactory fact, String host, int port, int timeout)
   ...
   * @throws me.legrange.mikrotik.MikrotikApiException Thrown if the API encounters an error on
   * login.
   */
   ...
   * @throws me.legrange.mikrotik.MikrotikApiException Thrown if the API encounters an error
   * executing a command.
   */
   ...
   * @throws me.legrange.mikrotik.MikrotikApiException Thrown if there is a problem cancelling the
   * command
   */
  public abstract void cancel(String tag) throws MikrotikApiException;

  /**
   * set the command timeout. The command timeout is used to time out API commands after a specific
   * time.
   *
   * Note: This is not the same as the timeout value passed in the connect() methods. This timeout
   * is specific to synchronous commands, that timeout is applied to opening the API socket.
   *
   ...
   *
   * @throws me.legrange.mikrotik.ApiConnectionException Thrown if there is a problem closing the
   * connection.
   * @since 2.2
   */
  @Override
